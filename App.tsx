  ...
  const handleSourceUpload = async (file: File) => {
    ...
    reader.onload = async (e) => {
      ...
      try {
        ...
        setErrorMessage(error.message || "Failed to extract data. Please ensure the file is clear and legible.");
        setSourceImage(null);
      } finally {
    ...
    <Layout currentStep={step}>
      ...
      {step === AppStep.UPLOAD_SOURCE && (
        <div className="space-y-8">
          {errorMessage && (
            <div className="max-w-2xl mx-auto bg-red-50 text-red-700 p-4 rounded-lg border border-red-200 flex items-center">
              <span className="font-semibold mr-2">Error:</span> {errorMessage}
            </div>
          )}
          <div className="text-center max-w-2xl mx-auto mb-10">
            <h1 className="text-3xl font-bold text-white mb-4 font-mono uppercase tracking-tight">Paperwork, Solved.</h1>
          ...
          <div className="flex justify-center mt-8">
            <button
              ...
              className="flex items-center space-x-2 text-slate-400 hover:text-white transition-colors bg-industrial-900 px-4 py-2 rounded-full border border-industrial-700 shadow-sm hover:border-industrial-orange font-mono"
            >
